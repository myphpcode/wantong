...
class User extends AuthController
{
    ...
    public function employee($uid)
    {
        ...
        $f[] = Form::input('account','员工账号');
        $f[] = Form::input('pwd','员工密码')->type('password');
        $f[] = Form::input('conf_pwd','确认密码')->type('password');
        $f[] = Form::input('real_name','真实姓名');

        $form = Form::make_post_form('修改其他', $f, Url::buildUrl('update_employee', array('uid' => $uid)));
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }

    public function update_employee($uid)
    {
...
//        if(!$data['roles']) return Json::fail('请选择至少一个管理员身份');
        ...
        if(AdminModel::be($uid,'uid')) return Json::fail('该员工已有账号');
...
//        $data['level'] = $this->adminInfo['level'] + 1;
        ...
        $res1 = UserModel::edit($edit, $uid);
        $res2 = Db::table('eb_system_admin')->insert($data);
        $res=$res1 && $res2;
        if( $res) {
    ...
    public function fenxiao($uid)
    {
        ...
        $f[] = Form::input('account','分销商账号');
        $f[] = Form::input('pwd','分销商密码')->type('password');
        $f[] = Form::input('conf_pwd','确认密码')->type('password');
        $f[] = Form::input('real_name','真实姓名');
        $form = Form::make_post_form('修改其他', $f, Url::buildUrl('update_fenxiao', array('uid' => $uid)));
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }

    public function update_fenxiao($uid)
    {
...
//        if(!$data['roles']) return Json::fail('请选择至少一个管理员身份');
        ...
        if(AdminModel::be($data['account'],'account')) return Json::fail('管理员账号已存在');
        if(AdminModel::be($uid,'uid')) return Json::fail('该分销商已有账号');
...
//        $data['level'] = $this->adminInfo['level'] + 1;
    ...
    public function admin_employee($uid)
    {
        if (!$uid) return $this->failed('数据不存在');
        $user = UserModel::get($uid);
//        $admin = $this->adminInfo;
        ...
        $f[] = Form::input('conf_pwd','确认密码')->type('password');
        $f[] = Form::input('real_name','真实姓名');
        $form = Form::make_post_form('修改其他', $f, Url::buildUrl('emp', array('uid' => $uid)));
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
  public function emp($uid)
    {
...
//        $data['level'] = $this->adminInfo['level'] + 1;
      ...
      $res = Db::table('eb_system_admin')->insert($data);
      if( $res) {
          return Json::successful('修改成功!');
...
//      dump(UserModel::getUserList($where));
        return Json::successlayui(UserModel::getUserList($where));
    }
    //推文员
    public function get_pUser_list()
    {
        $where = Util::getMore([
            ['page', 1],
            ['limit', 20],
...
//        $f[] = Form::input('spread_uid', '上级id', $user->getData('spread_uid'));
 ...
 if($user['spread_uid']!=$data['spread_uid']){
     //新
$newnum=UserModel::where('spread_uid',$data['spread_uid'])->count();
 UserModel::where('uid',$data['spread_uid'])->update(array('spread_count'=>$newnum));
 //老
     $oldnum=UserModel::where('spread_uid',$user['spread_uid'])->count();
     UserModel::where('uid',$user['spread_uid'])->update(array('spread_count'=>$oldnum));
