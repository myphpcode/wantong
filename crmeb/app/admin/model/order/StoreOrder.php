...
class StoreOrder extends BaseModel
{
    ...
    public static function orderCount()
    {
        ...
        //
        if (session('roles') == 3 || session('roles') == 2) {

            $wh['spread_uid'] = session('uid');
            $who['uid'] = session('uid');
            $d = Db::name('user')->where($wh)->whereOr($who)->column('uid');
            $map=[['uid','in',$d]];
        ...
        $data['wf'] = self::statusByWhere(1, new self())->where(['is_system_del' => 0, 'shipping_type' => 1])->where($map)->count();
        $data['ds'] = self::statusByWhere(2, new self())->where(['is_system_del' => 0, 'shipping_type' => 1])->where($map)->count();
        $data['dp'] = self::statusByWhere(3, new self())->where(['is_system_del' => 0])->where($map)->count();
        ...
        $data['general'] = self::where(['pink_id' => 0, 'combination_id' => 0, 'seckill_id' => 0, 'bargain_id' => 0, 'is_system_del' => 0])->where($map)->count();
        $data['pink'] = self::where('pink_id|combination_id', '>', 0)->where('is_system_del', 0)->where($map)->count();
        $data['seckill'] = self::where('seckill_id', '>', 0)->where('is_system_del', 0)->where($map)->count();
        $data['bargain'] = self::where('bargain_id', '>', 0)->where('is_system_del', 0)->where($map)->count();
        return $data;
    }
    ...
    public static function OrderList($where)
    {
        ...
        foreach ($data as $key=> &$item) {
            ...
            if($item['spread_uid']!=session('uid')&&$item['uid']!=session('uid')){
                unset($data[$key]);
            }
            //
...
//                    $model = $model->where($aler.'combination_id',">",0)->where($aler.'pink_id',">",0);
    ...
    public static function getOrderPrice($where)
    {
        ...
        $ids = self::getOrderWhere($where, $model)->where($whereData)->where($map)->column('id');
//        dump($ids);die;
        if (count($ids)) {
            $price['brokerage'] = UserBill::where(['category' => 'now_money', 'type' => 'brokerage'])->where('link_id', 'in', $ids)->sum('number');
        }
        $price['refund_price'] = self::getOrderWhere($where, $model)->where($map)->where(['is_del' => 0, 'paid' => 1, 'refund_status' => 2])->sum('refund_price');
        $sumNumber = self::getOrderWhere($where, $model)->where($whereData)->where($map)->field([
        ...
        $list = self::getOrderWhere($where, $model)->where($whereData)->where([$map])->group('pay_type')->column('sum(pay_price) as sum_pay_price,pay_type', 'id');
        foreach ($list as $v) {
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function payTypeCount()
    {
        ...
        $where['order'] = '';
        $where['pay_type'] = 1;
        $weixin = self::getOrderWhere($where, new self)->where($map)->count();
        $where['pay_type'] = 2;
        $yue = self::getOrderWhere($where, new self)->where($map)->count();
        $where['pay_type'] = 3;
        $offline = self::getOrderWhere($where, new self)->where($map)->count();
