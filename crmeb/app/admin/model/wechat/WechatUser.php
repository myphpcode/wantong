    ...
    public static function systemPage($where = array(),$isall=false){
     ...
     public static function setSpreadWhere($where=[],$alias='a',$model=null)
     {
         ...
         if ($status == 1) {
             //新加
             if(session('roles')==3)
             {$map=['spread_uid'=>session('uid')];}
             else{ $map=['is_promoter' => 1,'spread_uid'=>session('uid')];}
             //
             if ($Listuids = User::where($map)->field('uid')->select()) {
...
//                 $Listuids = self::getModelTime($where,User::where('spread_uid',session('uid')))->field('uid')->select();
//                 $newUids = [];
         ...
         foreach ($data as $key=>&$item) {
              if($item['spread_uid']!=session('uid')){unset($data[$key]);}
             else{
      ...
      * @return array
      */
     public static function employeePage($where = array()){
         $model=self::setSpreadWhere($where);
         $status =sysConfig('store_brokerage_statu');
         if(isset($where['excel']) && $where['excel'] == 1){
             $list = $model->field(['a.uid','u.phone','a.nickname','a.sex','a.country','a.province','a.city','a.now_money','a.subscribe','u.brokerage_price'])->select()->toArray();
